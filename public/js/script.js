$(document).ready(function(){
	...
	function testParse(data){
		for (var i = 0; i < data.test.length; i++){
			...
			for (var x = 0; x < data.test[i].answers.length; x++) {
				if (data.test[i].type == 'single') {
					...
					slotStart = ('<div class="form-group"><h6>' + data.test[i].question + '</h6>');
					...
					slotStart = ('<div class="form-group" data-total="' + totalVal + '"><h6>' + data.test[i].question + ' (Select ' + totalVal + ' )</h6>');
				}
			};
			var slotEnd = ('</div>');
		...
		$('#test-text .btn').on('click', function(){
			...
			$('#reset').html('<button>Try Again?</button>');
